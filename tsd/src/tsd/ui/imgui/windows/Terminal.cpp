...
void Terminal::buildUI()
{
  ...
  float inputHeight = ImGui::GetTextLineHeight() * lines
      + ImGui::GetStyle().FramePadding.y * 2;
  float footerHeight =
      ImGui::GetStyle().ItemSpacing.y + 1 + ImGui::GetStyle().ItemSpacing.y
      + inputHeight;
...
int Terminal::inputCallback(ImGuiInputTextCallbackData *data)
{
  ...
  if (data->EventFlag == ImGuiInputTextFlags_CallbackAlways) {
    bool altUp =
        ImGui::GetIO().KeyAlt && ImGui::IsKeyPressed(ImGuiKey_UpArrow);
    bool altDown =
    ...
    if (altUp || altDown) {
      ...
      if (prevPos != t->m_historyPos) {
        const char *str = (t->m_historyPos >= 0)
            ? t->m_history[t->m_historyPos].c_str()
            : "";
