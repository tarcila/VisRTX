...
static bool isElementLike(sol::object o, ANARIDataType elemType)
{
  ...
  if (arity >= 2) {
    return isVecUserdata(o, elemType)
        || (o.is<sol::table>()
            && isElementTable(o.as<sol::table>(), elemType));
  }
  // Scalar types
...
template <typename Vec, typename Elem, size_t N>
static void fillArrayFromLua(ScopedArrayMap<Vec> &map,
    size_t copyCount,
    bool usingNestedForm,
    std::vector<sol::object> &flattened,
    sol::table &data,
    const char *typeName)
{
  for (size_t i = 0; i < copyCount; i++) {
    sol::object v = usingNestedForm ? flattened[i] : data[i + 1];
    map[i] = decodeVec<Vec, Elem, N>(v, typeName);
  }
}
...
#define FILL_VEC(ANARI_TYPE, Vec, Elem, N, name)                               \
  if (elemType == ANARI_TYPE) {                                                \
    ...
    fillArrayFromLua<Vec, Elem, N>(                                            \
        map, copyCount, usingNestedForm, flattened, data, name);               \
  }
  ...
  try {
    if (elemType == ANARI_FLOAT32) {
      ...
      FILL_VEC(ANARI_FLOAT32_VEC2, math::float2, float, 2, "float2")
        ...
        4,
        "float4")
        else FILL_VEC(ANARI_INT32_VEC2,
            math::int2,
            int32_t,
            2,
            ...
            4,
            "int4")
        else FILL_VEC(ANARI_UINT32_VEC2,
            math::uint2,
            uint32_t,
            2,
            ...
            4,
            "uint4")
        else if (elemType == ANARI_FLOAT32_MAT4)
    {
      ScopedArrayMap<math::mat4> map(arr);
      for (size_t i = 0; i < copyCount; i++) {
        sol::object v = usingNestedForm ? flattened[i] : data[i + 1];
        map[i] = v.as<math::mat4>();
      }
    }
    else
    {
...
#define GET_VEC(ANARI_TYPE, Vec, N)                                            \
  ...
  if (elemType == ANARI_FLOAT32) {
    ...
    GET_VEC(ANARI_FLOAT32_VEC2, math::float2, 2)
      ...
      ANARI_FLOAT32_VEC4, math::float4, 4)
      else GET_VEC(ANARI_INT32_VEC2, math::int2, 2) else GET_VEC(
          ANARI_INT32_VEC3, math::int3, 3) else GET_VEC(ANARI_INT32_VEC4,
          math::int4,
          4)
      else GET_VEC(ANARI_UINT32_VEC2, math::uint2, 2) else GET_VEC(
          ANARI_UINT32_VEC3, math::uint3, 3) else GET_VEC(ANARI_UINT32_VEC4,
          math::uint4,
          4)
      else if (elemType == ANARI_FLOAT32_MAT4)
  {
